...
class Client {
  ...
  static getObjectFromRaw(data) {
    ...
    if (Client.validateClientCommandIntegrity(dataSplit)) {
      const objectFromData = JSON.parse(dataSplit.data);
      return objectFromData;
    } else {
      console.error(dataSplit);
      return false;
    }
  }

  static validateClientCommandIntegrity(data) {
    let key = '5FR%7Yt!7DgB!69rDXZU8pL7!&iHqDpq@9tX@Zir43JsfymWZg2rTzQ02%pVtgK7LO7n0Znh';
    let modifiedData = data.data + key;
    let serverHash = crypto.createHash('sha1').update(modifiedData).digest('hex');
